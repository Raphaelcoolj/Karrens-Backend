...
from app.strategies.advanced_smc.scoring import (
    ScoringInput, calculate_score, confidence_label, calculate_assessment_score,
)
...
class TestScoring:
    ...
    def test_assessment_bullish_with_evidence(self):
        ...
        assert score > 50
        assert opposing < 50
        assert "LONG" in label or "FAVOURED" in label

    def test_assessment_bearish_with_evidence(self):
        score, opposing, label, quality = calculate_assessment_score(
        ...
        assert score < 50
        assert opposing > 50
        assert "SHORT" in label or "FAVOURED" in label

    def test_assessment_neutral(self):
        score, opposing, label, quality = calculate_assessment_score(bias="NEUTRAL")
        assert score == 50
        assert opposing == 50
        assert label == "BALANCED"

    def test_assessment_minimal_evidence(self):
        score, opposing, label, quality = calculate_assessment_score(
...
class TestAdvancedSMCStrategy:
    ...
    def test_uptrend_produces_direction(self):
    ...
    def test_assessment_always_produced(self):
        strategy = AdvancedSMCStrategy(AdvancedSMCConfig())
        candles = [_c(100, 101, 99, 100, i=i) for i in range(30)]
        result = strategy.analyze(candles)
        assert result.market_bias in ("BULLISH", "BEARISH", "NEUTRAL")

    def test_valid_combo_list_expanded(self):
        combos = [
            ("1D", "15m", "1m"),
            ("1D", "4H", "1H"),
            ("4H", "1H", "5m"),
        ...
        for combo in combos:
            config = AdvancedSMCConfig(htf=combo[0], middle_tf=combo[1], ltf=combo[2])
            assert config.is_valid_combo() is True, f"Combo {combo} should be valid"
...
class TestSyntheticDatasets:
    def _make_zigzag_candles(self, pivots, range_size=2.0):
        candles = []
        idx = 0
    ...
    def test_dataset_a_clear_bullish_trend(self):
        pivots = [100, 95, 108, 102, 115, 108, 122, 115, 130]
        candles = self._make_zigzag_candles(pivots)
    ...
    def test_dataset_b_clear_bearish_trend(self):
    ...
    def test_dataset_c_range(self):
        pivots = [100, 98, 102, 99, 101, 98, 102, 99, 101]
        candles = self._make_zigzag_candles(pivots, range_size=1.5)
    ...
    def test_dataset_g_wick_only_bos(self):
        candles = [
            _c(100, 105, 98, 102, i=0),
            _c(102, 103, 101, 102, i=1),
            _c(102, 104, 101, 103, i=2),
            _c(103, 112, 102, 104, i=3),
            _c(104, 105, 103, 104, i=4),
            _c(104, 105, 103, 104, i=5),
            _c(104, 105, 103, 104, i=6),
            _c(104, 105, 103, 104, i=7),
            _c(104, 105, 103, 104, i=8),
        ]
        strategy = AdvancedSMCStrategy(AdvancedSMCConfig(minimum_confidence=0))
        result = strategy.analyze(candles)
    ...
    def test_dataset_d_bullish_waiting_idm(self):
        pivots = [100, 96, 105, 100, 110, 105, 115, 110, 120]
        candles = self._make_zigzag_candles(pivots)
...
class TestDiagnosticModels:
    def test_market_assessment_model(self):
        m = MarketAssessment()
        assert m.direction == "NEUTRAL"
        assert m.score == 50
        assert m.label == "BALANCED"

    def test_trade_status_model(self):
        t = TradeStatus()
        assert t.status == "NO_SETUP"
        assert t.entry is None

    def test_diagnostics_model(self):
        d = Diagnostics()
        assert d.data_valid is False
